 ...
 <el-row :gutter="15">
   ...
   <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title"  width="500px">
      <el-form ref="form"  :model="form" :rules="rules"  label-width="80px">
        ...
        <el-form-item label="数据权限"  v-show="form.dataScope == 2" prop="depts">
...
import {  changeRoleEnabled,  updateRole,  getRole } from '@/api/organization/role'
import {queryResource, fetchResourceByParentId, queryAllResource } from '@/api/organization/resource'
...
mixins: [presenter(), header(), form(defaultForm), crud()],
  data() {
    return {
      ...
      deptExpand: false,
      deptNodeAll: false,
      menuCheckStrictly: false, deptCheckStrictly: true,
       // 查询参数
      queryParams: {
        name: undefined,
        visible: undefined
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      ...
      getRole(this.currentId).then(response => {
        ...
        this.$refs.menu.setCheckedKeys(response.data.resourceIds)
      })
        .catch(() => {
          console.log('超时4')
        })
