  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        <!-- 搜索 -->
        <el-input v-model="query.name" clearable size="small" placeholder="资源名称" style="width: 200px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
    ...
    >
      ...
      <el-table-column :show-overflow-tooltip="true" label="资源名称" width="125px" prop="t.name" />
      ...
      <el-table-column v-permission="['admin']" label="操作" width="130px" align="center" fixed="right">
        <template slot-scope="scope">
          <udOperation
            :data="scope.row"
...
<script>
import crudResource from '@/api/organization/resource'
...
// crud交由presenter持有
const defaultForm = { id: null, name: null, parentId: null, code: null, type: null, url: null, method: null, description: null }
export default {
  name: 'ResourceManagement',
  components: { Treeselect, IconSelect, crudOperation, rrOperation, udOperation, DateRangePicker },
  cruds() {
    return CRUD({ title: '权限', url: '/organization/resource', crudMethod: { ...crudResource }})
  },
  ...
  methods: {
    ...
    getMenus(tree, treeNode, resolve) {
      ...
      setTimeout(() => {
        crudResource.getMenus(params).then(res => {
          resolve(res.content)
        })
      }, 100)
    },
    getSupDepts(id) {
      crudResource.getMenuSuperior(id).then(res => {
    ...
    loadMenus({ action, parentNode, callback }) {
      if (action === LOAD_CHILDREN_OPTIONS) {
        crudResource.getMenusTree(parentNode.id).then(res => {
