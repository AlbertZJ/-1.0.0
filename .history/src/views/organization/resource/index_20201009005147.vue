<template>
  <div class="app-container">
    ...
    >
      <el-table-column prop="t.name" label="资源名称" :show-overflow-tooltip="true"  ></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getTreeselect() {
      queryResource(this.queryParams).then(response => {
        this.menuOptions = []
        const resource = { id: 0, name: '主类目', children: [] }
  resource.children = this.handleTree(response.data, "id")
        this.menuOptions.push(resource)
      })
    },
    // 显示状态字典翻译
    ...
    handleAdd(row) {
      ...
      this.$alert(row.id)
      this.getTreeselect()
      if (row != null && row.t.id) {
        this.form.parentId = row.t.id
      } else {
    ...
    handleUpdate(row) {
      ...
      this.reset()
      this.getTreeselect()
      getMenu(row.t.id).then(response => {
