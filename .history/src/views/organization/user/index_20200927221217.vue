<template>
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :span="4" :xs="24">
        ...
        <div class="head-container">
          <el-tree :data="deptDatas"  :load="getDeptDatas" :props="defaultProps" :expand-on-click-node="false" :filter-node-method="filterNode" ref="tree" default-expand-all @node-click="handleNodeClick" />
        </div>
      </el-col>
      <!--用户数据-->
 ...
 <el-select
        ...
        <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
          ...
          <el-table-column :show-overflow-tooltip="true" prop="depts" label="部门">
            <template slot-scope="scope">
              <div>{{ scope.row.depts }}</div>
            </template>
          </el-table-column>
          <el-table-column :show-overflow-tooltip="true" prop="roles" label="角色">
            <template slot-scope="scope">
              <div>{{ scope.row.roles }}</div>
            </template>
          </el-table-column>
          <el-table-column :show-overflow-tooltip="true" prop="positions" label="职位">
            <template slot-scope="scope">
              <div>{{ scope.row.positions }}</div>
            </template>
          </el-table-column>
...
// import { getAllJob } from '@/api/organization/job'
...
let userRoles = []
let userJobs = []
const defaultForm = { id: null, username: null, nickname: null, sex: '男', email: null, enabled: 'false', roles: [], positions: [], depts: { id: null }, mobile: null }
export default {
  name: 'User',
  ...
  methods: {
      ...
      setTimeout(() => {
        getDepts(params).then(res => {
          if (resolve) {
            ...
            console.log("log",this.deptDates)
          }
        })
      }, 100)
    },
    ...
    changeEnabled(data, val) {
      this.$confirm('此操作将禁用"'  + data.username + '账号, 是否继续？', '提示', {
