    ...
    <!-- <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="65%">
      ...
      >
        <el-form-item label="字典数据名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入字典数据名称，如：首席执行官" maxlength="40" suffix-icon="el-icon-edit" clearable />
        </el-form-item>
...
const defaultForm = {
  ...
  listClass: null, isDefault: null, description: null, enabled: true,
  description: null
}
export default {
  ...
  data() {
    return {
      ...
      multiple: true,
      rules: {
        name: [{ required: true, message: '字典数据名称必填', trigger: 'blur' }]
      }
    }
  },
  computed: {
