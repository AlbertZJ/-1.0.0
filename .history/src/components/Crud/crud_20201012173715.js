...
function CRUD(options) {
  ...
  const methods = {
     ...
     */
    // submitSuccessNotify() {
    //   crud.notify(crud.mesg.submit, CRUD.NOTIFICATION_TYPE.SUCCESS)
    // },
    // addSuccessNotify() {
    //   crud.notify(crud.mesg.add, CRUD.NOTIFICATION_TYPE.SUCCESS)
    // },
    // editSuccessNotify() {
    //   crud.notify(crud.mesg.edit, CRUD.NOTIFICATION_TYPE.SUCCESS)
    // },
    // delSuccessNotify() {
    //   crud.notify(crud.mesg.del, CRUD.NOTIFICATION_TYPE.SUCCESS)
    // },
    // 搜索
    toQuery() {
      crud.page.current = 1
    ...
    doAdd() {
      ...
      crud.crudMethod.add(crud.form).then((response) => {
        console.log("code:",response.code)
        console.log("mesg:",response.mesg)
        crud.status.add = CRUD.STATUS.NORMAL
        crud.resetForm()
        crud.notify(response.mesg, CRUD.NOTIFICATION_TYPE.SUCCESS)
    ...
    doEdit() {
      ...
      crud.crudMethod.edit(crud.form).then((response) => {
        crud.status.edit = CRUD.STATUS.NORMAL
        crud.getDataStatus(crud.getDataId(crud.form)).edit = CRUD.STATUS.NORMAL
        crud.notify(response.mesg, CRUD.NOTIFICATION_TYPE.SUCCESS)
    ...
    doDelete(data) {
      ...
      return crud.crudMethod.del(ids).then((response) => {
        if (delAll) {
    ...
    getDataId(data) {
      console.log("you",data[this.idField])
