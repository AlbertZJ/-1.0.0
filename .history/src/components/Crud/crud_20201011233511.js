  ...
  const methods = {
   ...
   /**
    ...
    doDelete(data) {
      ...
      return zhuishu.crudMethod.del(ids).then(() => {
        if (delAll) {
    ...
    importTemplate() {
      crud.downloadLoading = true
      importTemplate(crud.url + '/excel/importTemplate', crud.getQueryParams()).then(result => {
...
function zhuishu(options = {}) {
  ...
  return {
    data() {
      return {
        zhuishu: this.zhuishu
      }
    },
    beforeCreate() {
      this.zhuishu = lookupCrud(this)
      this.zhuishu.registerVM(options.type, this)
    },
    destroyed() {
      this.zhuishu.unregisterVM(options.type, this)
    }
  }
}
