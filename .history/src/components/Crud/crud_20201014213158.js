...
function CRUD(options) {
  ...
  const methods = {
    ...
    doAdd() {
      ...
      crud.crudMethod.add(crud.form).then((response) => {
        ...
        if(response.code=="000000"){
          ...
          crud.notify(response.mesg, CRUD.NOTIFICATION_TYPE.WARNING)
        }
    ...
    dleChangePage(size) {
      console.log("crud.data.length:",crud.data.length)
      if (crud.data.length = size && crud.page.current != 1) {
        crud.page.current -= 1
        console.log("page:",crud.page.current)
      }
      // else{
